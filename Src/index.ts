...
export class Pass_Encr_Decr {
  constructor() {
    function ALPassWordEncrytpion(payload: PASSWORD) {
    ...
    function ALPassWordDecrytpion(payload: ENCRYPTION_PASSWORD) {
      const algorithm = "aes-256-cbc";
      const security_key = crypto
        .createHmac("sha256", payload.secret_init_vector)
        .update(payload.secret_key)
    ...
    function ALVerifyPassWord(payload: VERIFY_PASSWORD) {
      const algorithm = "aes-256-cbc";
      const security_key = crypto
        .createHmac("sha256", payload.secret_init_vector)
        .update(payload.secret_key)
